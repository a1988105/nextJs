...
export default async function MarketNewsPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-12">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/[0.07]">
                <th className="text-left px-5 py-3.5 text-gray-600 font-medium text-xs uppercase tracking-wider">#</th>
                <th className="text-left px-5 py-3.5 text-gray-600 font-medium text-xs uppercase tracking-wider">Coin</th>
                <th className="text-right px-5 py-3.5 text-gray-600 font-medium text-xs uppercase tracking-wider">Price</th>
                <th className="text-right px-5 py-3.5 text-gray-600 font-medium text-xs uppercase tracking-wider">24h</th>
            ...
            <tbody>
              {coins.map((coin) => {
                ...
                return (
                  ...
                  >
                    <td className="px-5 py-4 num text-gray-600 text-xs">{coin.market_cap_rank}</td>
                    <td className="px-5 py-4">
                      ...
                      >
                        ...
                        <span className="num text-gray-600 uppercase text-xs">{coin.symbol}</span>
                      </Link>
                    </td>
                    ...
                    <td className="px-5 py-4 text-right">
                      <span
                        className={`inline-block num text-xs px-2 py-0.5 rounded font-medium ${
                          isPos ? 'text-emerald-400 bg-emerald-400/10' : 'text-red-400 bg-red-400/10'
                        }`}
                      >
                        {isPos ? '+' : ''}
      ...
      <p className="num text-xs text-gray-700 mt-5">↻ Revalidates every 60 seconds via ISR</p>
    </div>
  )
}
